...
#$AppTitle = "01iverrozario's Windows Utility"
...
$WPFinstall.Add_Click({
        ...
        If ( $WPFInstallpython3.IsChecked -eq $true ) { 
            $wingetinstall.Add("Python.Python.3.10")
            $WPFInstallpython3.IsChecked = $false
        }
        IF ($WPFInstallpython2.IsChecked -eq $true){
            $wingetinstall.Add("Python.Python.2")
            $WPFInstallpython2.IsChecked = $false
        }
        ...
        IF($WPFInstallRustGNU.IsChecked -eq $true){
            $wingetinstall.Add("Rustlang.Rust.GNU")
            $WPFInstallRustGNU.IsChecked = $false
        }
        ...
        if ($wingetinstall.Count -eq 0) {
            ...
            [System.Windows.MessageBox]::Show($WarningMsg, $ProgramName, [System.Windows.MessageBoxButton]::OK, [System.Windows.MessageBoxImage]::Warning)
            return
        }
...
$WPFInstallUpgrade.Add_Click({
        ...
        try {
            ...
            start-process winget -verb runas -ArgumentList "upgrade --all"-Wait -WindowStyle Maximized 
...
$WPFFeatureInstall.Add_Click({
        ...
        If ( $WPFWinSandbox.IsChecked -eq $true ){
            Enable-WindowsOptionalFeature -FeatureName "Containers-DisposableClientVM" -All -Online -NoRestart  
            Write-Host "Windows sandbox is installed"
        }
        If ($WPFTelnet.IsChecked -eq $true){
            Enable-WindowsOptionalFeature -Online -FeatureName "TelnetClient"
        }
        #$NoButton = [System.Windows.MessageBoxButton]::NO
        $OpButton = [System.Windows.MessageBoxButton]::OK
        $MessageboxTitle = "Config"
        $Messageboxbody = ("Features are now installed. Please restart your device.")
        $MessageIcon = [System.Windows.MessageBoxImage]::Information

        [System.Windows.MessageBox]::Show($Messageboxbody, $MessageboxTitle, $OpButton, $MessageIcon)
        Write-Host "================================="
    ...
    $WPFFeatureUninstall.Add_Click({
        ...
        $OpButton = [System.Windows.MessageBoxButton]::OK
        $MessageboxTitle = "Config"
        $Messageboxbody = ("Features are now uninstalled. Please restart your device.")
        $MessageIcon = [System.Windows.MessageBoxImage]::Information
